...
<<<<<<< HEAD
    ...
    public void select() {
        ...
        /* Case: filtered customer list, select index within bounds of address book but out of bounds of customer list
         * -> rejected
         */
        showCustomersWithName(KEYWORD_MATCHING_MEIER);
        int invalidIndex = getModel().getAddressBook().getCustomerList().size();
        assertCommandFailure(SelectCommand.COMMAND_WORD + " " + invalidIndex, MESSAGE_INVALID_CUSTOMER_DISPLAYED_INDEX);

        /* Case: filtered customer list, select index within bounds of address book and customer list -> selected */
